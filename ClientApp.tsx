...
import { Monitor, Wifi, Activity, Lock, Terminal, X, Power, ShieldCheck, Maximize, Minimize, Bell } from 'lucide-react';
...
const ClientApp: React.FC = () => {
  // Determine Execution Mode from URL
  const params = new URLSearchParams(window.location.search);
  const mode = params.get('mode'); // 'client' (browser), 'worker' (electron-bg), 'overlay' (electron-ui)
  ...
  const [isActive, setIsActive] = useState(mode === 'worker' || mode === 'overlay'); // Auto-active in Electron modes
  ...
  const addToast = (title: string, message: string, logo: string, isUrgent: boolean, duration: number, x?: number, y?: number) => {
    ...
    // Auto-dismiss logic for duration > 0
    if (duration > 0) {
        setTimeout(() => {
            setToasts(prev => {
                const updated = prev.filter(t => t.id !== newToast.id);
                // If this was the last toast, we need to make window click-through again in overlay mode
                if (updated.length === 0 && mode === 'overlay' && (window as any).electronAPI) {
    ...
    // In Overlay Mode, if we add a toast, we must capture mouse events
    if (mode === 'overlay' && (window as any).electronAPI) {
  ...
  // --- EFFECT: WORKER MODE (Background Listener) ---
  useEffect(() => {
      if (mode !== 'worker') return;
      console.log("[Worker] Started background listener...");

      // Default Station ID if none set
      let currentId = stationId;
      if (!currentId) {
          currentId = "STATION-DESKTOP"; 
      ...
      const unsubscribe = onSnapshot(q, (snapshot) => {
          ...
          if (logId !== lastLogIdRef.current) {
              ...
              if (targets.includes('ALL') || (myId && targets.includes(myId))) {
                   console.log("[Worker] Alert received, sending to main process...");
                   const payload = {
                   ...
                   if ((window as any).electronAPI) {
                       (window as any).electronAPI.sendNotification(payload);
                   }
              }
          }
      });
  ...
  // --- EFFECT: OVERLAY MODE (UI Display) ---
  useEffect(() => {
      if (mode !== 'overlay') return;
      console.log("[Overlay] Ready to display...");
      
      if ((window as any).electronAPI) {
          (window as any).electronAPI.onShowToast((data: any) => {
  ...
  // --- EFFECT: BROWSER CLIENT (Original Logic) ---
  useEffect(() => {
      if (mode === 'worker' || mode === 'overlay') return; // Skip if electron
      if (!isActive) return;
  ...
  if (mode === 'worker') {
      return (
          <div className="flex items-center justify-center h-screen bg-black text-green-500 font-mono text-xs p-4">
              <div>
                  <h1 className="font-bold">CYBER CORP BACKGROUND SERVICE</h1>
                  <p>Status: RUNNING</p>
                  <p>Station ID: {stationId}</p>
                  <p className="mt-2 animate-pulse">Listening for Firestore events...</p>
              </div>
          </div>
      );
  }
  ...
  // --- RENDER: OVERLAY MODE ---
  if (mode === 'overlay') {
      // Transparent container that holds toasts
      return (
          <div className="w-screen h-screen overflow-hidden flex flex-col items-end justify-end p-6 pointer-events-none">
              {/* Toasts - Fixed Position Bottom Right */}
              <div className="flex flex-col-reverse gap-3 items-end">
                  {toasts.map(toast => (
                      <div 
                          key={toast.id}
                          className={`
                              pointer-events-auto w-[360px] rounded border shadow-2xl flex flex-col overflow-hidden transition-all animate-in slide-in-from-right-full duration-300
                              ${toast.isUrgent ? 'bg-[#1a0505] border-red-900/50' : 'bg-[#1f1f1f] border-[#333]'}
                          `}
                      >
                          {/* Header */}
                          <div className={`flex items-center px-3 py-2 ${toast.isUrgent ? 'bg-red-950/80' : 'bg-[#2b2b2b]'}`}>
                              {toast.logo && <img src={toast.logo} className="w-4 h-4 mr-2 object-contain rounded-sm" />}
                              <span className="text-xs font-bold text-white mr-auto">System Notification</span>
                              {toast.duration === 0 && (
                                <button 
                                    onClick={() => handleToastClose(toast.id, toast.isUrgent, toast)}
  ...
  return (
      <div className="relative h-screen w-full bg-[#0b1120] text-slate-300 overflow-hidden flex flex-col items-center justify-center">
          ...
          {/* Browser Toasts (Only for fallback) */}
          <div className="fixed bottom-5 right-5 z-50 flex flex-col-reverse gap-3 pointer-events-none">
              {toasts.map(toast => (
                  <div key={toast.id} className="pointer-events-auto w-[360px] rounded border shadow-2xl flex flex-col overflow-hidden bg-[#1f1f1f] border-[#333]">
                      <div className="flex items-center px-3 py-2 bg-[#2b2b2b]">
                          <span className="text-xs font-bold text-white mr-auto">System Notification</span>
                          <button onClick={() => handleToastClose(toast.id, toast.isUrgent, toast)} className="text-slate-400 hover:text-white p-1"><X size={14} /></button>
                      </div>
                      <div className="p-4 flex gap-3"><div className="flex-1"><h4 className="text-sm font-bold text-white mb-1">{toast.title}</h4><p className="text-xs text-slate-300 leading-relaxed">{toast.message}</p></div></div>
                  </div>
              ))}
          </div>
      </div>
